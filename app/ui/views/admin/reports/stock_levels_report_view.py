...
class StockLevelsReportView(ft.Container):
    def __init__(self, page: ft.Page, router, current_user: User, license_status: bool,
        ...
        self.game_filter_dropdown = ft.Dropdown(label="Filter by Game", hint_text="All Active Games", on_change=self._on_game_filter_change, width=300, border_radius=8, options=[ft.dropdown.Option(key="", text="All Active Games")])
        ...
        column_definitions: List[Dict[str, Any]] = [
            ...
            # active_stock_value from crud_reports is sum of Book.remaining_value, now in CENTS
            {"key": "active_stock_value", "label": "Active Stock Val ($)", "sortable": True, "numeric": True,
             "display_formatter": lambda val_cents, item: ft.Text(f"{(val_cents / 100.0):.2f}" if val_cents is not None else "$0.00", weight=ft.FontWeight.BOLD, size=12.5)},
        ]
        self.report_table = PaginatedDataTable[Dict[str, Any]](page=self.page, fetch_all_data_func=lambda: self.report_data_cache, column_definitions=column_definitions, action_cell_builder=None, rows_per_page=15, initial_sort_key="game_name", initial_sort_ascending=True, no_data_message="No stock data found. Adjust filter and click 'Generate Report'.")
        self.page.appbar = create_appbar(page=self.page, router=self.router, title_text=f"{self.current_user.role.capitalize()} > Stock Levels Report", current_user=self.current_user, license_status=self.license_status, leading_widget=ft.IconButton(ft.Icons.ARROW_BACK_IOS_NEW_ROUNDED, tooltip="Go Back", icon_color=ft.Colors.WHITE, on_click=self._go_back))
    ...
    def _generate_report_data_and_display(self, e: ft.ControlEvent = None):
        ...
        try:
            with get_db_session() as db: self.report_data_cache = self.report_service.get_stock_levels_report_data(db, self._selected_game_id_filter)
    ...
    def _update_summary_totals(self):
        # active_stock_value in report_data_cache is in CENTS
        total_value_cents = sum(item.get('active_stock_value', 0) for item in self.report_data_cache)
        self.summary_total_active_stock_value.value = f"Total Active Stock Value: ${(total_value_cents / 100.0):.2f}" # Display in dollars
    ...
    def _export_report_to_pdf(self, e: ft.ControlEvent):
        ...
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S"); game_filter_name_suffix = "AllActiveGames"; game_filter_display_name = "All Active Games"
        if self._selected_game_id_filter:
            ...
            if selected_game: game_filter_name_suffix = f"Game{selected_game.game_number}"; game_filter_display_name = f"{selected_game.game_number} - {selected_game.name}"
