...
class GameExpiryReportView(ft.Container):
    def __init__(self, page: ft.Page, router, current_user: User, license_status: bool,
        ...
        self._selected_status_filter: Optional[str] = None
        ...
        self.status_filter_dropdown = ft.Dropdown(label="Filter by Status", hint_text="All Statuses", on_change=self._on_status_filter_change, width=200, border_radius=8, options=[ft.dropdown.Option(key="", text="All Statuses"), ft.dropdown.Option(key="active", text="Active"), ft.dropdown.Option(key="expired", text="Expired"),])
        ...
        column_definitions: List[Dict[str, Any]] = [
            ...
            {"key": "price", "label": "Price ($)", "sortable": True, "numeric": True,
             "display_formatter": lambda val_cents, item: ft.Text(f"{(val_cents / 100.0):.2f}" if val_cents is not None else "", size=12.5)},
        ...
        self.report_table = PaginatedDataTable[Dict[str, Any]](page=self.page, fetch_all_data_func=lambda: self.report_data_cache, column_definitions=column_definitions, action_cell_builder=None, rows_per_page=15, initial_sort_key="is_expired", initial_sort_ascending=False, no_data_message="No games found. Adjust filters and click 'Generate Report'.")
        self.page.appbar = create_appbar(page=self.page, router=self.router, title_text=f"{self.current_user.role.capitalize()} > Game Expiry Report", current_user=self.current_user, license_status=self.license_status, leading_widget=ft.IconButton(ft.Icons.ARROW_BACK_IOS_NEW_ROUNDED, tooltip="Go Back", icon_color=ft.Colors.WHITE, on_click=self._go_back))
    ...
    def _on_status_filter_change(self, e: ft.ControlEvent):
        ...
        self.expired_start_date_button.disabled = not is_expired_selected; self.expired_end_date_button.disabled = not is_expired_selected
        if not is_expired_selected:
            self._selected_expired_start_date = None; self._selected_expired_end_date = None
            self.expired_start_date_button.text = "Expired Start"; self.expired_end_date_button.text = "Expired End"
    ...
    def _on_date_selected(self, e: ft.ControlEvent, date_type: str):
        ...
        selected_dt = e.control.value; picker.open = False
        if selected_dt:
        ...
        if button.page: button.update(); self._validate_dates();
    ...
    def _generate_report_data_and_display(self, e: ft.ControlEvent = None):
        ...
        end_dt = datetime.datetime.combine(self._selected_expired_end_date, datetime.time.max) if self._selected_expired_end_date else None # Use time.max for end_dt
        try:
            with get_db_session() as db: self.report_data_cache = self.report_service.get_game_expiry_report_data(db, self._selected_status_filter, start_dt, end_dt)
            self.report_table.refresh_data_and_ui(); self.export_pdf_button.disabled = not bool(self.report_data_cache)
        except Exception as ex: self._show_error(f"Error generating report: {ex}")
    ...
    def _build_body(self) -> ft.Container:
        filters_row = ft.Row([self.status_filter_dropdown, self.expired_start_date_button, self.expired_end_date_button, self.generate_report_button, self.export_pdf_button], alignment=ft.MainAxisAlignment.START, vertical_alignment=ft.CrossAxisAlignment.CENTER, spacing=10, wrap=True)
        report_card_content = ft.Column([ft.Text("Game Expiry Report Filters", style=ft.TextThemeStyle.TITLE_LARGE, weight=ft.FontWeight.BOLD), filters_row, self.error_text_widget, ft.Divider(height=15), ft.Text("Report Results", style=ft.TextThemeStyle.TITLE_LARGE, weight=ft.FontWeight.BOLD), self.report_table,], spacing=15, expand=True, scroll=ft.ScrollMode.ADAPTIVE,)
