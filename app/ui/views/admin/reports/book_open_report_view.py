...
from app.services import ReportService # UserService not directly needed here unless for user context
...
class BookOpenReportView(ft.Container):
    def __init__(self, page: ft.Page, router, current_user: User, license_status: bool,
        ...
        self.game_filter_dropdown = ft.Dropdown(
            label="Filter by Game", hint_text="All Games",
        ...
        column_definitions: List[Dict[str, Any]] = [
            ...
            {"key": "game_price_per_ticket", "label": "Tkt Price", "sortable": False, "numeric": True, "display_formatter": lambda val, item: ft.Text(f"${val:.2f}" if val is not None else "$0.00", size=12.5)},
            {"key": "remaining_value", "label": "Rem. Value", "sortable": True, "numeric": True, "display_formatter": lambda val, item: ft.Text(f"${val:.2f}" if val is not None else "$0.00", size=12.5, weight=ft.FontWeight.BOLD)},
        ]
        ...
        self.report_table = PaginatedDataTable[Dict[str, Any]](
            ...
            no_data_message="No open books found. Adjust filter and click 'Generate Report'."
        )
    ...
    def _load_initial_filters(self):
        try:
            with get_db_session() as db:
                self.all_games_for_filter = self.report_service.get_all_games_for_filter(db)
            options = [ft.dropdown.Option(key="", text="All Games")]
            options.extend([ft.dropdown.Option(key=str(game.id), text=f"{game.game_number} - {game.name}") for game in self.all_games_for_filter if not game.is_expired]) # Show only active games for selection
            ...
            self._show_error(f"Error loading game filters: {e}")
    ...
    def _generate_report_data_and_display(self, e: ft.ControlEvent = None):
        ...
        try:
            with get_db_session() as db:
                self.report_data_cache = self.report_service.get_book_open_report_data(db, self._selected_game_id_filter)
    ...
    def _update_summary_totals(self):
        ...
        total_remaining_value = sum(item.get('remaining_value', 0) for item in self.report_data_cache)
        self.summary_total_open_books.value = f"Total Open Books: {total_open_books}"
        self.summary_total_remaining_value.value = f"Total Remaining Value: ${total_remaining_value:.2f}"
    ...
    def _export_report_to_pdf(self, e: ft.ControlEvent):
        if not self.report_data_cache:
            self.page.open(ft.SnackBar(ft.Text("No data to export."), open=True))
            return
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        game_filter_name = "AllGames"
    ...
    def _on_file_picker_result(self, e: ft.FilePickerResultEvent):
        if e.path:
            save_path = e.path
            game_filter_display_name = "All Games"
            if self._selected_game_id_filter:
                selected_game = next((g for g in self.all_games_for_filter if g.id == self._selected_game_id_filter), None)
                if selected_game: game_filter_display_name = f"{selected_game.game_number} - {selected_game.name}"

            success, msg = self.report_service.generate_book_open_report_pdf(
                report_data=self.report_data_cache, game_filter_name=game_filter_display_name, pdf_save_path=save_path
            )
    ...
    def _show_error(self, message: str):
        self.error_text_widget.value = message
    ...
    def _build_body(self) -> ft.Container:
        ...
        summary_row = ft.Row(
            [self.summary_total_open_books, ft.Container(width=20), self.summary_total_remaining_value],
            alignment=ft.MainAxisAlignment.END, spacing=20
        )
        report_card_content = ft.Column(
            [
                ft.Text("Open Books Report Filters", style=ft.TextThemeStyle.TITLE_LARGE, weight=ft.FontWeight.BOLD),
