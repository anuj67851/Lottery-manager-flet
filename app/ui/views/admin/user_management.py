...
from app.constants import EMPLOYEE_ROLE, MANAGED_USER_ROLES, ADMIN_DASHBOARD_ROUTE, ADMIN_ROLE # Added ADMIN_ROLE
...
class AdminUserManagementView(ft.Container):
    def __init__(self, page: ft.Page, router, current_user: User, license_status: bool,
        ...
        self.search_bar = SearchBarComponent(on_search_changed=self._on_search_term_changed, label="Search Users (Username, Role)", expand=True)
        self.page.appbar = create_appbar(
            page=self.page, router=self.router, title_text=f"{self.current_user.role.capitalize()} > User Management",
            current_user=self.current_user, license_status=self.license_status,
            leading_widget=ft.IconButton(icon=ft.Icons.ARROW_BACK_IOS_NEW_ROUNDED, tooltip="Go Back to Admin Dashboard", icon_color=ft.Colors.WHITE, on_click=self._go_back)
        )
        self.users_table_component = UsersTable(
            page=self.page, user_service=self.user_service, initial_roles_to_display=MANAGED_USER_ROLES,
            current_acting_user=self.current_user, on_data_changed_callback=self._handle_table_data_change
        )
    ...
    def _open_add_user_dialog(self):
        ...
        role_options = [ft.dropdown.Option(role, role.capitalize()) for role in MANAGED_USER_ROLES] # Admin can create Admin or Employee
    ...
    def _build_body(self) -> ft.Container:
        ...
        user_management_card_content = ft.Column([ft.Row([ft.Text("Account Management", style=ft.TextThemeStyle.HEADLINE_SMALL, weight=ft.FontWeight.BOLD, expand=True), self.search_bar, ft.FilledButton("Add New User", icon=ft.Icons.PERSON_ADD_ALT_1_ROUNDED, on_click=self._handle_add_user_click, style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=8)), height=48)], alignment=ft.MainAxisAlignment.SPACE_BETWEEN, vertical_alignment=ft.CrossAxisAlignment.CENTER), ft.Divider(height=15), self.users_table_component], spacing=15, expand=True)
        ...
        card_effective_width = min(TARGET_CARD_MAX_WIDTH, page_width_for_calc - 40); card_effective_width = max(card_effective_width, 600)
        user_management_card = ft.Card(content=ft.Container(content=user_management_card_content, padding=20, border_radius=ft.border_radius.all(10)), elevation=2, width=card_effective_width)
        return ft.Container(content=user_management_card, alignment=ft.alignment.top_center, padding=20, expand=True)