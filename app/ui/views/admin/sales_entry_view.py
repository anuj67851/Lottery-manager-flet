...
from app.core.models import User, ShiftSubmission
...
class SalesEntryView(ft.Container):
    def __init__(self, page: ft.Page, router, current_user: User, license_status: bool,
        ...
        self.scanner_text_field: ft.TextField = ft.TextField(label="Scan Full Book Code (Game+Book+Ticket)", hint_text=f"Input {MIN_REQUIRED_SCAN_LENGTH_WITH_TICKET} chars total.", autofocus=True, border_radius=8, prefix_icon=ft.Icons.QR_CODE_SCANNER_ROUNDED, expand=True, height=50 )
    ...
    def _clear_scan_error_properties(self):
        if self.scan_error_text_widget.visible:
            self.scan_error_text_widget.value = ""; self.scan_error_text_widget.visible = False
    ...
    def _update_totals_and_book_counts_properties(self, changed_item_data: Optional[SalesEntryItemData] = None):
        ...
        # amount_calculated_cents is now in cents
        grand_total_instant_sales_cents = sum(item.amount_calculated_cents for item in all_display_items if item.is_processed_for_sale or item.all_tickets_sold_confirmed)
        ...
        self.grand_total_sales_widget.value = f"Grand Total Instant Sales: ${(grand_total_instant_sales_cents / 100.0):.2f}" # Display in dollars
    ...
    def _on_scan_complete_callback(self, parsed_data: Dict[str, str]):
        self._clear_scan_error_properties()
        game_no_str = parsed_data.get('game_no', ''); book_no_str = parsed_data.get('book_no', '')
    ...
    def _on_scan_error_callback(self, error_message: str):
        self.scan_error_text_widget.value = error_message; self.scan_error_text_widget.visible = True
        if self.scan_error_text_widget.page: self.scan_error_text_widget.update()
    ...
    def _process_scan_and_update_table(self, game_no_str: str, book_no_str: str, ticket_no_str: str):
        self._clear_scan_error_properties()
        try:
            with get_db_session() as db: book_model_instance = self.sales_entry_service.get_or_create_book_for_sale(db, game_no_str, book_no_str)
            if self.sales_items_table_component: self.sales_items_table_component.add_or_update_book_for_sale(book_model_instance, ticket_no_str)
            else: raise Exception("Sales items table component not initialized.")
        except (ValidationError, GameNotFoundError, BookNotFoundError, DatabaseError) as e: self._on_scan_error_callback(str(e.message if hasattr(e, 'message') else e))
        except Exception as ex_general: self._on_scan_error_callback(f"Error processing scan: {type(ex_general).__name__} - {ex_general}")
        if self.scan_input_handler: self.scan_input_handler.focus_input()
        if self.page: self.page.update()
    ...
    def _handle_submit_shift_sales_click(self, e):
        ...
        try:
            # NumberDecimalField returns float for money, int if integer_only=True for SR34. Service expects floats.
        ...
        self._clear_scan_error_properties()
        # get_data_for_submission now returns 'amount_calculated_cents'
        ...
        if items_with_empty_fields:
            self._prompt_for_empty_field_books_confirmation(items_with_empty_fields, reported_online_sales_float, reported_online_payouts_float, reported_instant_payouts_float, actual_cash_in_drawer_float, instant_sales_items_data_for_submission )
        else:
        ...
        def _handle_dialog_choice(mark_as_all_sold: bool):
            ...
            if mark_as_all_sold:
                for item_data in items_to_confirm:
                    ...
                    if self.sales_items_table_component: self.sales_items_table_component.update_datarow_for_item(item_data.unique_id)
                    updated_sales_item_details.append(item_data.get_data_for_submission()) # Will include amount_calculated_cents
            self._update_totals_and_book_counts_properties()
            self._open_confirm_shift_submission_dialog(reported_online_sales_float, reported_online_payouts_float, reported_instant_payouts_float, actual_cash_in_drawer_float, updated_sales_item_details )
            if self.page: self.page.update()
        confirm_dialog = ft.AlertDialog(modal=True, title=ft.Text("Confirm Unentered Instant Sale Books"), content=dialog_content_column, actions=[ ft.TextButton("Cancel Submission", on_click=lambda _: self.page.close(self.page.dialog)), ft.TextButton("No, Skip These Empty", on_click=lambda _: _handle_dialog_choice(False)), ft.FilledButton("Yes, Mark All Sold", on_click=lambda _: _handle_dialog_choice(True)), ], actions_alignment=ft.MainAxisAlignment.END, )
        self.page.dialog = confirm_dialog; self.page.open(confirm_dialog)

    def _open_confirm_shift_submission_dialog( self, reported_online_sales_float: float, reported_online_payouts_float: float, reported_instant_payouts_float: float, actual_cash_in_drawer_float: float, sales_item_details: List[Dict[str, Any]]):
        total_instant_tickets = sum(item.get('tickets_sold_calculated', 0) for item in sales_item_details)
        total_instant_value_cents = sum(item.get('amount_calculated_cents', 0) for item in sales_item_details) # Sum cents
        total_instant_value_dollars = total_instant_value_cents / 100.0 # Convert to dollars for display

        confirmation_content_column = ft.Column([ ft.Text("Please confirm the values you are submitting:", weight=ft.FontWeight.BOLD), ft.Divider(), ft.Text(f"Reported Total Online Sales Today (SR 50): ${reported_online_sales_float:.2f}"), ft.Text(f"Reported Total Online Payouts Today (SR 50): ${reported_online_payouts_float:.2f}"), ft.Text(f"Reported Total Instant Payouts Today (SR 34): ${reported_instant_payouts_float:.2f}"), ft.Text(f"Lottery cash in Drawer: ${actual_cash_in_drawer_float:.2f}", weight=ft.FontWeight.BOLD, color=ft.Colors.BLUE_700, size=15), ft.Divider(), ft.Text("Instant Game Sales for this submission (from table):"), ft.Text(f"  Total Instant Tickets: {total_instant_tickets}"), ft.Text(f"  Total Instant Value: ${total_instant_value_dollars:.2f}"), ft.Divider(), ft.Text("This will finalize this set of entries. Are you sure?", weight=ft.FontWeight.BOLD)], tight=True, spacing=8, width=450, scroll=ft.ScrollMode.AUTO, height=370)
        final_confirm_dialog = create_confirmation_dialog(title_text="Confirm Shift Submission", content_control=confirmation_content_column, on_confirm=lambda e: self._execute_database_submission(reported_online_sales_float, reported_online_payouts_float, reported_instant_payouts_float, actual_cash_in_drawer_float, sales_item_details), on_cancel=lambda ev: self.page.close(self.page.dialog), confirm_button_text="Submit Shift", confirm_button_style=ft.ButtonStyle(bgcolor=ft.Colors.GREEN_ACCENT_700, color=ft.Colors.WHITE), )
        self.page.dialog = final_confirm_dialog; self.page.open(final_confirm_dialog)

    def _execute_database_submission( self, reported_online_sales_float: float, reported_online_payouts_float: float, reported_instant_payouts_float: float, actual_cash_in_drawer_float: float, sales_item_details: List[Dict[str, Any]]):
        self.page.close(self.page.dialog) # type: ignore
        ...
        loading_banner = ft.Banner(bgcolor=ft.Colors.AMBER_100, leading=ft.Icon(ft.Icons.HOURGLASS_TOP_ROUNDED, color=ft.Colors.AMBER_900, size=30), content=ft.Text("Submitting shift and sales data, please wait...", color=ft.Colors.AMBER_900, weight=ft.FontWeight.BOLD), actions=[ft.TextButton("Dismiss", disabled=True)] )
        self.page.banner = loading_banner; self.page.banner.open = True; self.page.update()
        try:
            with get_db_session() as db:
                submitted_shift = self.shift_service.create_new_shift_submission(db=db, user_id=self.current_user.id, reported_online_sales_float=reported_online_sales_float, reported_online_payouts_float=reported_online_payouts_float, reported_instant_payouts_float=reported_instant_payouts_float, actual_cash_in_drawer_float=actual_cash_in_drawer_float, sales_item_details=sales_item_details )
            self.page.banner.open = False; self._open_submission_summary_dialog(submitted_shift); self._load_initial_data_for_table()
    ...
    def _open_submission_summary_dialog(self, submitted_shift: ShiftSubmission):
        # All values from submitted_shift are in CENTS
        ...
        total_instant_value_dollars = submitted_shift.total_value_instant / 100.0 # total_value_instant is now in CENTS

        diff_text_val = f"${abs(drawer_difference_dollars):.2f}"; diff_label = ""; diff_color = ft.Colors.BLACK
    ...
    def _go_to_dashboard(self, e):
        self.page.close(self.page.dialog); self.page.dialog = None; self._go_back(e)

    def _build_body(self) -> ft.Container:
        self.scan_input_handler = ScanInputHandler(scan_text_field=self.scanner_text_field, on_scan_complete=self._on_scan_complete_callback, on_scan_error=self._on_scan_error_callback, require_ticket=True )
        self.sales_items_table_component = SalesEntryItemsTable(page_ref=self.page, sales_entry_service=self.sales_entry_service, on_item_change_callback=self._update_totals_and_book_counts_properties, on_all_items_loaded_callback=self._handle_table_items_loaded )
        reported_totals_row1 = ft.Row([self.reported_online_sales_field, self.reported_online_payouts_field], spacing=10)
        reported_totals_row2 = ft.Row([self.reported_instant_payouts_field, self.actual_cash_in_drawer_field], spacing=10)
        info_row = ft.Row([ self.today_date_widget, ft.Container(expand=True), self.books_in_table_count_widget, ft.VerticalDivider(width=1, color=ft.Colors.with_opacity(0.3, ft.Colors.ON_SURFACE), thickness=1), self.pending_entry_books_count_widget, ], alignment=ft.MainAxisAlignment.START, spacing=15, vertical_alignment=ft.CrossAxisAlignment.CENTER )
        top_section = ft.Column([ info_row, ft.Text("Enter Cumulative Daily Totals from External Terminal & Actual Cash:", style=ft.TextThemeStyle.TITLE_SMALL, weight=ft.FontWeight.BOLD), reported_totals_row1, reported_totals_row2, ft.Divider(height=10, color=ft.Colors.TRANSPARENT), ft.Text("Scan Instant Game Tickets or Enter Next Ticket # Manually:", style=ft.TextThemeStyle.TITLE_SMALL, weight=ft.FontWeight.BOLD), ft.Row([self.scanner_text_field], vertical_alignment=ft.CrossAxisAlignment.CENTER), self.scan_error_text_widget ], spacing=10 )
        bottom_summary_section = ft.Container(ft.Row([ self.total_tickets_sold_widget, self.grand_total_sales_widget, ft.FilledButton( "Submit Shift Sales", icon=ft.Icons.SAVE_AS_ROUNDED, on_click=self._handle_submit_shift_sales_click, style=ft.ButtonStyle(bgcolor=ft.Colors.GREEN_700, color=ft.Colors.WHITE, shape=ft.RoundedRectangleBorder(radius=8)), height=45, tooltip="Finalize and submit this shift's sales data." )], alignment=ft.MainAxisAlignment.SPACE_BETWEEN, vertical_alignment=ft.CrossAxisAlignment.CENTER ), padding=ft.padding.symmetric(vertical=15, horizontal=5) )
        sales_card_content = ft.Column([top_section, ft.Divider(height=15), self.sales_items_table_component, ft.Divider(height=15), bottom_summary_section], spacing=12, expand=True )
        ...
        card_effective_width = min(TARGET_CARD_MAX_WIDTH, page_width_for_calc - 40); card_effective_width = max(card_effective_width, 750)
        sales_card = ft.Card(content=ft.Container(content=sales_card_content, padding=ft.padding.symmetric(vertical=15, horizontal=20), border_radius=ft.border_radius.all(10)), elevation=3, width=card_effective_width, )
