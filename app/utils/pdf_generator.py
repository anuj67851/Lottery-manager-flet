...
class PDFGenerator:
    ...
    def setup_styles(self):
        ...
        try: base_font = 'Helvetica'; bold_font = 'Helvetica-Bold'
        ...
        self.styles.add(ParagraphStyle(name='TableHeader', fontName=bold_font, fontSize=7.5, leading=10, alignment=TA_CENTER, textColor=colors.whitesmoke))
        self.styles.add(ParagraphStyle(name='TableCell', fontName=base_font, fontSize=7.5, leading=10, alignment=TA_LEFT))
        self.styles.add(ParagraphStyle(name='TableCellRight', fontName=base_font, fontSize=7.5, leading=10, alignment=TA_RIGHT))
        self.styles.add(ParagraphStyle(name='TableCellCenter', fontName=base_font, fontSize=7.5, leading=10, alignment=TA_CENTER))
    ...
    def generate_summary_table(self, data: List[List[Paragraph]], column_widths: Optional[List[float]] = None):
        if not data: self.story.append(Paragraph("No summary data available.", self.styles['FilterInfo'])); return
        table = Table(data, colWidths=column_widths)
        style = TableStyle([('GRID', (0, 0), (-1, -1), 0.5, colors.grey), ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'), ('LEFTPADDING', (0, 0), (-1, -1), 6), ('RIGHTPADDING', (0, 0), (-1, -1), 6), ('BACKGROUND', (0, -1), (-1, -1), colors.lightgrey if "Drawer Difference" in data[-1][0].text or "Calculated Drawer" in data[-1][0].text else None), ('TEXTCOLOR', (0, -1), (0, -1), colors.darkblue if "Drawer Difference" in data[-1][0].text or "Calculated Drawer" in data[-1][0].text else colors.black), ('FONTNAME', (0, -1), (-1, -1), 'Helvetica-Bold' if "Drawer Difference" in data[-1][0].text or "Calculated Drawer" in data[-1][0].text else 'Helvetica'),])
    ...
    def generate_shifts_summary_table(self, data: List[Dict[str, Any]], column_headers: List[str], column_widths: Optional[List[float]] = None):
        # Data items monetary values (deltas, total_value_instant, calculated_drawer_value, drawer_difference) are in CENTS
        if not data: self.story.append(Paragraph("No shift submission data available.", self.styles['FilterInfo'])); return
        if "Cum. Diff ($)" not in column_headers: # Check for new column header name
            try:
                drawer_diff_idx = column_headers.index("Drawer Diff. ($)")
                column_headers.insert(drawer_diff_idx + 1, "Cum. Diff ($)")
                if column_widths and len(column_widths) == len(column_headers) - 1:
                    new_width = column_widths[drawer_diff_idx] * 0.8; column_widths[drawer_diff_idx] *= 0.8; column_widths.insert(drawer_diff_idx + 1, new_width)
            except ValueError:
                column_headers.append("Cum. Diff. ($)")
                if column_widths: column_widths.append(0.08 * (self.page_size[0] - 1*inch))

        table_data = [[Paragraph(header, self.styles['TableHeader']) for header in column_headers]]
        totals_cents = {key: 0 for key in ["calculated_delta_online_sales", "calculated_delta_online_payouts", "calculated_delta_instant_payouts", "total_value_instant", "calculated_drawer_value", "drawer_difference"]}
        totals_other = {"total_tickets_sold_instant": 0}
        cumulative_drawer_difference_cents = 0
        sorted_data = sorted(data, key=lambda x: x.get('submission_datetime', datetime.datetime.min))

        for item in sorted_data:
            drawer_diff_cents_item = item.get('drawer_difference', 0)
            cumulative_drawer_difference_cents += drawer_diff_cents_item
            diff_dollars = drawer_diff_cents_item / 100.0; diff_text = f"${abs(diff_dollars):.2f}"; diff_lbl = " (S)" if diff_dollars > 0 else (" (O)" if diff_dollars < 0 else "")
            cum_diff_dollars_item = cumulative_drawer_difference_cents / 100.0; cum_text = f"${abs(cum_diff_dollars_item):.2f}"; cum_lbl = " (S)" if cum_diff_dollars_item > 0 else (" (O)" if cum_diff_dollars_item < 0 else "")
            ...
            row = [ Paragraph(item.get('submission_datetime').strftime('%Y-%m-%d %H:%M') if item.get('submission_datetime') else '', self.styles['TableCellCenter']),
                    ...
                    Paragraph(f"${(item.get('total_value_instant', 0)/100.0):.2f}", self.styles['TableCellRight']), # total_value_instant is CENTS
                    Paragraph(f"${(item.get('calculated_drawer_value', 0)/100.0):.2f}", self.styles['TableCellRight']),
                    Paragraph(f"{diff_text}{diff_lbl}", self.styles['TableCellRight']),
                    Paragraph(f"{cum_text}{cum_lbl}", self.styles['TableCellRight']), ]
        ...
        total_drawer_diff_dollars = totals_cents['drawer_difference'] / 100.0; total_diff_text = f"${abs(total_drawer_diff_dollars):.2f}"; total_diff_lbl = " (S)" if total_drawer_diff_dollars > 0 else (" (O)" if total_drawer_diff_dollars < 0 else "")
        final_cum_diff_dollars = cumulative_drawer_difference_cents / 100.0; final_cum_text = f"${abs(final_cum_diff_dollars):.2f}"; final_cum_lbl = " (S)" if final_cum_diff_dollars > 0 else (" (O)" if final_cum_diff_dollars < 0 else "")

        totals_row_paragraphs = [ Paragraph("<b>Totals:</b>", self.styles['SummaryTotal']), '', '',
                                  ...
                                  Paragraph(f"<b>${(totals_cents['total_value_instant']/100.0):.2f}</b>", self.styles['SummaryTotal']), # total_value_instant is CENTS
                                  Paragraph(f"<b>${(totals_cents['calculated_drawer_value']/100.0):.2f}</b>", self.styles['SummaryTotal']),
                                  Paragraph(f"<b>{total_diff_text}{total_diff_lbl}</b>", self.styles['SummaryTotal']),
                                  Paragraph(f"<b>{final_cum_text}{final_cum_lbl}</b>", self.styles['SummaryTotal']),]
        table_data.append(totals_row_paragraphs)
        if not column_widths or len(column_widths) != len(column_headers): page_w, _ = self.page_size; avail_w = page_w - 1*inch; num_c = len(column_headers); def_cw = avail_w / num_c if num_c > 0 else 1*inch; column_widths = [def_cw] * num_c
        table = Table(table_data, colWidths=column_widths, repeatRows=1)
        style = TableStyle([ ('BACKGROUND', (0,0), (-1,0), colors.darkslateblue), ('TEXTCOLOR',(0,0),(-1,0),colors.whitesmoke), ('FONTNAME', (0,0), (-1,0), 'Helvetica-Bold'), ('ALIGN', (0,0), (-1,0), 'CENTER'), ('GRID', (0,0), (-1,-1), 0.5, colors.grey), ('LINEBELOW', (0,0), (-1,0), 1, colors.darkblue), ('ROWBACKGROUNDS', (0,1), (-1,-2), [colors.aliceblue, colors.lavender]), ('VALIGN', (0,0), (-1,-1), 'MIDDLE'), ('BACKGROUND', (0,-1), (-1,-1), colors.lightgrey), ('LINEABOVE', (0,-1), (-1,-1), 1, colors.darkblue), ('SPAN', (0,-1), (2,-1)), ])
    ...
    def generate_sales_report_table(self, data: List[Dict[str, Any]], column_headers_ignored: List[str], column_widths_ignored: Optional[List[float]] = None):
        # Data items: ticket_price_actual (Game.price) and sales_entry_total_value (SalesEntry.price) are in CENTS
        ...
        total_qty, total_value_cents = 0, 0
        for item in data:
            ...
            row = [ Paragraph(dt_str, self.styles['TableCellCenter']), Paragraph(user_shift_str, self.styles['TableCell']),
                    ...
                    Paragraph(f"${(item.get('ticket_price_actual', 0)/100.0):.2f}", self.styles['TableCellRight']), # Convert cents to dollars
                    Paragraph(f"${(item.get('sales_entry_total_value', 0)/100.0):.2f}", self.styles['TableCellRight']), ] # Convert cents to dollars
            table_data.append(row)
            total_qty += item.get('count', 0); total_value_cents += item.get('sales_entry_total_value', 0)

        qty_idx = actual_headers.index("Qty"); total_idx = actual_headers.index("Total ($)")
        totals_row_vals = [Paragraph("<b>Totals (for listed entries):</b>", self.styles['SummaryTotal'])] + [''] * (qty_idx -1) + [Paragraph(f"<b>{total_qty}</b>", self.styles['SummaryTotal'])] + [''] * (total_idx - qty_idx -1) + [Paragraph(f"<b>${(total_value_cents/100.0):.2f}</b>", self.styles['SummaryTotal'])] # Convert total to dollars
        ...
        style = TableStyle([ ('BACKGROUND', (0,0), (-1,0), colors.cadetblue), ('TEXTCOLOR',(0,0),(-1,0),colors.whitesmoke), ('FONTNAME', (0,0), (-1,0), 'Helvetica-Bold'), ('ALIGN', (0,0), (-1,0), 'CENTER'), ('GRID', (0,0), (-1,-1), 0.5, colors.grey), ('LINEBELOW', (0,0), (-1,0), 1, colors.darkblue), ('ROWBACKGROUNDS', (0,1), (-1,-2), [colors.whitesmoke,colors.lightcyan]), ('VALIGN', (0,0), (-1,-1), 'MIDDLE'), ('BACKGROUND', (0,-1), (-1,-1), colors.lightgrey), ('LINEABOVE', (0,-1), (-1,-1), 1, colors.darkblue), ('SPAN', (0,-1), (qty_idx -1,-1)), ])
        alignments_str = ['CENTER', 'LEFT', 'LEFT', 'CENTER', 'CENTER', 'RIGHT', 'RIGHT', 'RIGHT', 'RIGHT', 'RIGHT']
        for i, align_val_str in enumerate(alignments_str): style.add('ALIGN', (i,1), (i,-2), align_val_str)
        if 0 <= qty_idx < len(actual_headers): style.add('ALIGN', (qty_idx,-1), (qty_idx,-1), 'RIGHT')
        if 0 <= total_idx < len(actual_headers): style.add('ALIGN', (total_idx,-1), (total_idx,-1), 'RIGHT')
        table.setStyle(style); self.story.append(KeepTogether(table))
    ...
    def generate_book_open_report_table(self, data: List[Dict[str, Any]], column_headers: List[str], column_widths: Optional[List[float]] = None):
        # Data items: game_price_per_ticket and remaining_value are in CENTS
        ...
        for item in data:
            ...
            row = [ Paragraph(str(item.get('game_name','')), self.styles['TableCell']), Paragraph(str(item.get('game_number','')), self.styles['TableCellCenter']),
                    ...
                    Paragraph(f"${(item.get('game_price_per_ticket',0)/100.0):.2f}", self.styles['TableCellRight']), # Convert cents to dollars
                    Paragraph(f"${(item.get('remaining_value',0)/100.0):.2f}", self.styles['TableCellRight']), ] # Convert cents to dollars
            table_data.append(row); grand_total_val_cents += item.get('remaining_value',0)
        totals_row = [Paragraph("<b>Grand Total Rem. Value:</b>", self.styles['SummaryTotal'])] + ['']*(len(column_headers)-2) + [Paragraph(f"<b>${(grand_total_val_cents/100.0):.2f}</b>", self.styles['SummaryTotal'])] # Convert total to dollars
        ...
        style.add('ALIGN',(0,1),(0,-2),'LEFT'); table.setStyle(style); self.story.append(KeepTogether(table))
    ...
    def generate_game_expiry_report_table(self, data: List[Dict[str, Any]], column_headers: List[str], column_widths: Optional[List[float]] = None):
        # Data item: Game.price is in CENTS
        ...
        for item in data:
            ...
            row = [ Paragraph(str(item.get('name','')), self.styles['TableCell']), Paragraph(str(item.get('game_number','')), self.styles['TableCellCenter']),
                    Paragraph(f"${(item.get('price',0)/100.0):.2f}", self.styles['TableCellRight']), # Convert cents to dollars
        ...
        style.add('ALIGN',(0,1),(0,-1),'LEFT'); table.setStyle(style); self.story.append(KeepTogether(table))
    ...
    def generate_stock_levels_report_table(self, data: List[Dict[str, Any]], column_headers: List[str], column_widths: Optional[List[float]] = None):
        # Data items: game_price_per_ticket and active_stock_value are in CENTS
        ...
        grand_total_val_cents = 0
        for item in data:
            act_val_cents = item.get('active_stock_value',0); grand_total_val_cents += act_val_cents
            row = [ Paragraph(str(item.get('game_name','')), self.styles['TableCell']), Paragraph(str(item.get('game_number','')), self.styles['TableCellCenter']),
                    Paragraph(f"${(item.get('game_price_per_ticket',0)/100.0):.2f}", self.styles['TableCellRight']), # Convert cents to dollars
                    ...
                    Paragraph(f"${(act_val_cents/100.0):.2f}", self.styles['TableCellRight']), ] # Convert cents to dollars
            table_data.append(row)
        totals_row = [Paragraph("<b>Grand Total Active Stock Value:</b>", self.styles['SummaryTotal'])] + ['']*(len(column_headers)-2) + [Paragraph(f"<b>${(grand_total_val_cents/100.0):.2f}</b>", self.styles['SummaryTotal'])] # Convert total to dollars
        table_data.append(totals_row)
        if not column_widths: page_w,_=self.page_size; avail_w=page_w-1*inch; num_c=len(column_headers); def_cw=avail_w/num_c if num_c>0 else 1*inch; column_widths=[def_cw]*num_c
        table = Table(table_data, colWidths=column_widths, repeatRows=1)
        style = TableStyle([('BACKGROUND',(0,0),(-1,0),colors.cornflowerblue),('TEXTCOLOR',(0,0),(-1,0),colors.whitesmoke),('FONTNAME',(0,0),(-1,0),'Helvetica-Bold'),('ALIGN',(0,0),(-1,0),'CENTER'),('GRID',(0,0),(-1,-1),0.5,colors.grey),('LINEBELOW',(0,0),(-1,0),1,colors.darkblue),('ROWBACKGROUNDS',(0,1),(-1,-2),[colors.whitesmoke,colors.lightgrey]),('VALIGN',(0,0),(-1,-1),'MIDDLE'),('BACKGROUND',(0,-1),(-1,-1),colors.lightsteelblue),('LINEABOVE',(0,-1),(-1,-1),1,colors.darkblue),('SPAN',(0,-1),(len(column_headers)-2,-1)),('ALIGN',(len(column_headers)-1,-1),(len(column_headers)-1,-1),'RIGHT')])
